...
#define REMOVE_PERM(x) ((unsigned int) x & 0xfffff000)
...
{
    unsigned int addr = page_index * PAGESIZE;
...
{
    unsigned int *pt = (unsigned int *) REMOVE_PERM(PDirPool[proc_index][pde_index]);
    return pt[pte_index];
}
