 ...
 * In mCertiKOS, we statically allocate page directories, and maintain the
 * second level page tables dynamically. The unsigned int * type is meant to
 * suggest that the contents of the array are pointers to page tables. In
 * reality they are actually page directory entries, which are essentially
 * pointers plus permission bits. The functions in this layer will require
 * casting between integers and pointers anyway and in fact any 32-bit type is
 * fine, so feel free to change it if it makes more sense to you with a
 * different type.
 */
...
// Sets the CR3 register with the start address of the page structure for
// process # [index].
...
void set_pdir_entry(unsigned int proc_index, unsigned int pde_index,
  ...
  // TODO
  unsigned int addr = (page_index << 12) | PT_PERM_PTU;
...
// Removes the specified page directory entry (sets the page directory entry to
// 0). Don't forget to cast the value to (unsigned int *).
...
// Do not forget that the permission info is also stored in the page directory
// entries.
unsigned int get_ptbl_entry(unsigned int proc_index, unsigned int pde_index,
                            unsigned int pte_index) {
  // TODO
  unsigned int *pte_addr =
      (unsigned int *)((unsigned int)PDirPool[proc_index][pde_index] &
                       0xfffff000);
  return pte_addr[pte_index];
}

// Sets the specified page table entry with the start address of physical page #
// [page_index] You should also set the given permission.
void set_ptbl_entry(unsigned int proc_index, unsigned int pde_index,
  ...
  unsigned int *pte_addr =
      (unsigned int *)((unsigned int)PDirPool[proc_index][pde_index] &
                       0xfffff000);
  pte_addr[pte_index] = (page_index << 12) | perm;
}
...
void set_ptbl_entry_identity(unsigned int pde_index, unsigned int pte_index,
                             unsigned int perm) {
  // TODO
  IDPTbl[pde_index][pte_index] = ((pde_index << 22) | (pte_index << 12));
