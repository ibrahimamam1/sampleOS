...
#define GET_PDE_ADDR(x) (x / (4096 * 1024) )
#define GET_PTE_ADDR(x) ((x / 4096) & 0x3ff)
...
{
    unsigned int pde_index = GET_PDE_ADDR(vaddr);
    if (get_pdir_entry(proc_index, pde_index) != 0) {
        return get_ptbl_entry(proc_index, pde_index, GET_PTE_ADDR(vaddr));
  }
    return 0;
}
...
{
    return get_pdir_entry(proc_index, GET_PDE_ADDR(vaddr));
}
...
void rmv_ptbl_entry_by_va(unsigned int proc_index, unsigned int vaddr)
{
    unsigned int pde_index = GET_PDE_ADDR(vaddr);
    if (get_pdir_entry(proc_index, pde_index) != 0) {
        rmv_ptbl_entry(proc_index, pde_index, GET_PTE_ADDR(vaddr));
    }
}
...
{
    rmv_pdir_entry(proc_index, GET_PDE_ADDR(vaddr));
}
...
{
    set_ptbl_entry(proc_index, GET_PDE_ADDR(vaddr), GET_PTE_ADDR(vaddr), page_index, perm);
}
...
{
    set_pdir_entry(proc_index, GET_PDE_ADDR(vaddr), page_index);
}
