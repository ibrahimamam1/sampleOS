...
void pmem_init(unsigned int mbi_addr)
{
     ...
     */
   rows = get_size();
   highest_addr = get_mms(rows-1) + get_mml(rows-1) - 1;
   nps = highest_addr / PAGESIZE; 
    ...
    for(i=VM_USERLO_PI; i<VM_USERHI_PI; i++){
      unsigned int start = get_mms(i);
      unsigned int length = get_mml(i);
      unsigned int stop = start + length;
      int usable = 1; 
