...
void container_init(unsigned int mbi_addr) {
  ...
  /**
   * TODO: Compute the available quota and store it into the variable
   * real_quota. It should be the number of the unallocated pages with the
   * normal permission in the physical memory allocation table.
   */

  unsigned int count_norm = 0;
  for (int i = 0; i < num_pages; i++) {
    if (!at_is_allocated(i) && at_is_norm(i)) {
      count_norm++;
    }
  }
  real_quota = count_norm;
...
unsigned int container_can_consume(unsigned int id, unsigned int n) {
  // TODO
  if (n + CONTAINER[id].usage <= CONTAINER[id].quota)
    return 1;
...
unsigned int container_split(unsigned int id, unsigned int quota) {
  ...
  /**
   * TODO: Update the container structure of both parent and child process
   * appropriately.
   */
...
/**
 * Allocates one more page for process # [id], given that this will not exceed
 * the quota. The container structure should be updated accordingly after the
 * allocation. Returns the page index of the allocated page, or 0 in the case of
 * failure.
 */
unsigned int container_alloc(unsigned int id) {
  ...
  if (CONTAINER[id].usage + 1 <= CONTAINER[id].quota) {
    unsigned int page_location = id + CONTAINER[id].usage;
    palloc();
